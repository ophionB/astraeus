...
public final class CombatPrayer {
   ...
   */
  public static final ImmutableMap<Integer, PrayerType> VALUES =
      ImmutableMap.copyOf(Stream.of(PrayerType.values())
          .collect(Collectors.toMap(p -> p.getButtonId(), Function.identity())));
  ...
  public static enum PrayerType {
    THICK_SKIN("Thick Skin", 0, 5609, 0, 12.0, 83, 446, -1, PrayerGroup.DEFENCE), BURST_OF_STRENGTH(
        "Burst of Strength", 1, 5610, 4, 12.0, 84, 449, -1,
        PrayerGroup.STRENGTH), CLARITY_OF_THOUGHT("Clarity of Thought", 2, 5611, 7, 12.0, 85, 436,
            -1, PrayerGroup.ATTACK), SHARP_EYE("Sharp Eye", 3, 19812, 8, 12.0, 700, -1, -1,
                PrayerGroup.RANGED), MYSTIC_WILL("Mystic Will", 4, 19814, 9, 12.0, 701, -1, -1,
                    PrayerGroup.MAGIC), ROCK_SKIN("Rock Skin", 5, 5612, 10, 8.0, 86, 441, -1,
                        PrayerGroup.DEFENCE), SUPERHUMAN_STRENGTH("Superhuman Strength", 6, 5613,
                            13, 8.0, 87, 434, -1, PrayerGroup.STRENGTH), IMPROVED_REFLEXES(
                                "Improved Reflexes", 7, 5614, 16, 8.0, 88, 448, -1,
                                PrayerGroup.ATTACK), RAPID_RESTORE("Rapid Restore", 8, 5615, 19,
                                    60.0, 89, 452, -1, PrayerGroup.DEFAULT), RAPID_HEAL(
                                        "Rapid Heal", 9, 5616, 22, 60.0, 90, 443, -1,
                                        PrayerGroup.DEFAULT), PROTECT_ITEM("Protect Item", 10, 5617,
                                            25, 30.0, 91, -1, -1, PrayerGroup.DEFAULT), HAWK_EYE(
                                                "Hawk Eye", 11, 19816, 26, 6.0, 702, -1, -1,
                                                PrayerGroup.RANGED), MYSTIC_LORE("Mystic Lore", 12,
                                                    19818, 27, 6.0, 703, -1, -1,
                                                    PrayerGroup.MAGIC), STEEL_SKIN("Steel Skin", 13,
                                                        5618, 28, 6.0, 92, 439, -1,
                                                        PrayerGroup.DEFENCE), ULTIMATE_STRENGTH(
                                                            "Ultimate Strength", 14, 5619, 31, 6.0,
                                                            93, 450, -1,
                                                            PrayerGroup.STRENGTH), INCREDIBLE_REFLEXES(
                                                                "Incredible Reflexes", 15, 5620, 34,
                                                                6.0, 94, 440, -1,
                                                                PrayerGroup.ATTACK), PROTECT_FROM_MAGIC(
                                                                    "Protect from Magic", 16, 5621,
                                                                    37, 4.0, 95, 438, 2,
                                                                    PrayerGroup.HEAD_ICON), PROTECT_FROM_RANGE(
                                                                        "Protect from Range", 17,
                                                                        5622, 40, 4.0, 96, 444, 1,
                                                                        PrayerGroup.HEAD_ICON), PROTECT_FROM_MELEE(
                                                                            "Protect from Melee",
                                                                            18, 5623, 43, 4.0, 97,
                                                                            433, 0,
                                                                            PrayerGroup.HEAD_ICON), EAGLE_EYE(
                                                                                "Eagle Eye", 19,
                                                                                19821, 44, 6.0, 704,
                                                                                -1, -1,
                                                                                PrayerGroup.RANGED), MYSTIC_MIGHT(
                                                                                    "Mystic Might",
                                                                                    20, 19823, 45,
                                                                                    6.0, 705, -1,
                                                                                    -1,
                                                                                    PrayerGroup.MAGIC), RETRIBUTION(
                                                                                        "Retribution",
                                                                                        21, 683, 46,
                                                                                        4.0, 98, -1,
                                                                                        3,
                                                                                        PrayerGroup.HEAD_ICON), REDEMPTION(
                                                                                            "Redemption",
                                                                                            22, 684,
                                                                                            49, 3.0,
                                                                                            99, -1,
                                                                                            5,
                                                                                            PrayerGroup.HEAD_ICON), SMITE(
                                                                                                "Smite",
                                                                                                23,
                                                                                                685,
                                                                                                52,
                                                                                                ...
                                                                                                -1,
                                                                                                4,
                                                                                                PrayerGroup.HEAD_ICON), CHIVALRY(
                                                                                                    "Chivalry",
                                                                                                    24,
                                                                                                    19825,
                                                                                                    60,
    ...
    /**
     * The immutable list of all prayers that belong to the over head group and only one can be
     * activated at a given time.
     */
    static final ImmutableSet<PrayerType> HEAD_ICON_DISABLED = ImmutableSet.of(
        PrayerType.PROTECT_FROM_MAGIC, PrayerType.PROTECT_FROM_RANGE, PrayerType.PROTECT_FROM_MELEE,
        PrayerType.RETRIBUTION, PrayerType.REDEMPTION, PrayerType.SMITE);

    /**
     * The immutable list of all prayers that belong to the defense group and only one can be
     * activated at a given time.
     */
    static final ImmutableSet<PrayerType> DEFENCE_DISABLED = ImmutableSet.of(PrayerType.THICK_SKIN,
        PrayerType.ROCK_SKIN, PrayerType.STEEL_SKIN, PrayerType.CHIVALRY, PrayerType.PIETY);

    /**
     * The immutable list of all prayers that belong to the attack group and only one can be
     * activated at a given time.
     */
    static final ImmutableSet<PrayerType> ATTACK_DISABLED = ImmutableSet.of(
        PrayerType.CLARITY_OF_THOUGHT, PrayerType.IMPROVED_REFLEXES, PrayerType.INCREDIBLE_REFLEXES,
        PrayerType.SHARP_EYE, PrayerType.HAWK_EYE, PrayerType.EAGLE_EYE, PrayerType.MYSTIC_WILL,
        PrayerType.MYSTIC_LORE, PrayerType.MYSTIC_MIGHT, PrayerType.CHIVALRY, PrayerType.PIETY);

    /**
     * The immutable list of all prayers that belong to the strength group and only one can be
     * activated at a given time.
     */
    static final ImmutableSet<PrayerType> STRENGTH_DISABLED = ImmutableSet.of(
        PrayerType.BURST_OF_STRENGTH, PrayerType.SUPERHUMAN_STRENGTH, PrayerType.ULTIMATE_STRENGTH,
        PrayerType.SHARP_EYE, PrayerType.HAWK_EYE, PrayerType.EAGLE_EYE, PrayerType.MYSTIC_WILL,
        PrayerType.MYSTIC_LORE, PrayerType.MYSTIC_MIGHT, PrayerType.CHIVALRY, PrayerType.PIETY);

    /**
     * The immutable list of all prayers that belong to the attack and strength group and only one
     * can be activated at a given time.
     */
    static final ImmutableSet<PrayerType> ATTACK_STRENGTH_DISABLED = ImmutableSet.of(
        PrayerType.CLARITY_OF_THOUGHT, PrayerType.IMPROVED_REFLEXES, PrayerType.INCREDIBLE_REFLEXES,
        PrayerType.BURST_OF_STRENGTH, PrayerType.SUPERHUMAN_STRENGTH, PrayerType.ULTIMATE_STRENGTH,
        PrayerType.SHARP_EYE, PrayerType.HAWK_EYE, PrayerType.EAGLE_EYE, PrayerType.MYSTIC_WILL,
        PrayerType.MYSTIC_LORE, PrayerType.MYSTIC_MIGHT, PrayerType.CHIVALRY, PrayerType.PIETY);
    ...
    /**
     * The immutable list of all prayers that belong to the combat group and only one can be
     * activated at a given time.
     */
    static final ImmutableSet<PrayerType> COMBAT_DISABLED = ImmutableSet.of(
        PrayerType.CLARITY_OF_THOUGHT, PrayerType.IMPROVED_REFLEXES, PrayerType.INCREDIBLE_REFLEXES,
        PrayerType.BURST_OF_STRENGTH, PrayerType.SUPERHUMAN_STRENGTH, PrayerType.ULTIMATE_STRENGTH,
        PrayerType.THICK_SKIN, PrayerType.ROCK_SKIN, PrayerType.STEEL_SKIN, PrayerType.SHARP_EYE,
        PrayerType.HAWK_EYE, PrayerType.EAGLE_EYE, PrayerType.MYSTIC_WILL, PrayerType.MYSTIC_LORE,
        PrayerType.MYSTIC_MIGHT, PrayerType.CHIVALRY, PrayerType.PIETY);
    ...
    private PrayerType(String name, int id, int buttonId, int level, double drainRate, int configId,
        int soundId, int headIcon, PrayerGroup group) {
      this.name = name;
      this.id = id;
  ...
  private enum PrayerGroup {
    DEFENCE, STRENGTH, ATTACK, HEAD_ICON, MAGIC, RANGED, COMBAT, DEFAULT
  }
   ...
   *
   * @param player The player who owns these prayers.
   */
  ...
  private boolean canToggle(PrayerType prayer) {
    ...
    if (player.getSkills().getMaxLevel(Skill.PRAYER) < prayer.getRequiredLevel()) {
      player.getDialogueFactory()
          .sendStatement(String.format(
              "You need a Prayer level of <col=255>%d</col> to use <col=255>%s</col>.",
              prayer.getRequiredLevel(), prayer.getName()))
          .execute();
      return false;
    }
    ...
    if (prayer == PrayerType.CHIVALRY && player.getSkills().getMaxLevel(Skill.DEFENCE) < 65) {
      player.getDialogueFactory()
          .sendStatement("You need a Defence level of <col=255>65</col> to use Chivalry.")
          .execute();
      return false;
    }
    ...
    if (prayer == PrayerType.PIETY && player.getSkills().getMaxLevel(Skill.DEFENCE) < 70) {
      player.getDialogueFactory()
          .sendStatement("You need a Defence level of <col=255>70</col> to use Piety.").execute();
      return false;
    }
  ...
  /**
   * The method that will cause a players prayer level to drain based upon which prayers are
   * activated.
   */
  public void drain() {
    int amount = 0;
  ...
  public double getAffectedDrainRate(PrayerType prayer) {
    final int prayerBonus = 0;// player.getBonuses()[Equipment.PRAYER];
   ...
   */
  public void disable() {
    CombatPrayer.VALUES.forEach((b, p) -> {
      if (active(p))
        toggle(p, false);
    });
  }
