...
import astraeus.net.packet.out.UpdateSpecialAmountPacket;
...
public final class WeaponSpecial {
	 ...
	 *            the weapon type used when performing this special attack.
	 */
	public WeaponSpecial(SpecialType type, int[] ids, int amount, double strength, double accuracy, CombatType combat,
			WeaponType weapon) {
		this.type = type;
		this.ids = ids;
		this.amount = amount;
	...
	public static void update(Player player) {
		WeaponType type = player.getWeapon();
		...
		for (int i = 0; i < 10; i++) {
			player.queuePacket(new UpdateSpecialAmountPacket(--specialBar, specialAmount >= specialCheck ? 500 : 0));
			specialCheck--;
		}
