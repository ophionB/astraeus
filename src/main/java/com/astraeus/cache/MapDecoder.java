...
import com.astraeus.game.world.Region;
import com.astraeus.game.world.entity.object.GameObject;
import com.astraeus.net.codec.ByteBufUtils;
import com.astraeus.net.codec.game.ByteBufReader;
import com.astraeus.util.FileUtils;
...
public final class MapDecoder {
  ...
  public static void load() {
    try {
      ...
      for (MapDefinition definition : definitions) {
        ...
        byte[] objectData = FileUtils.decompressStoreEntry(
            new File("./data/map/mapdata/" + definition.getObjectFileId() + ".gz"));
        byte[] terrainData = FileUtils.decompressStoreEntry(
            new File("./data/map/mapdata/" + definition.getTerrainFileId() + ".gz"));
        ...
        try {
          MapDecoder.decodeRegion(definition.getRegionId(), ByteBufReader.wrap(objectData),
              ByteBufReader.wrap(terrainData));
          successfull++;
        } catch (Exception e) {
          e.printStackTrace();
          System.out.println("Error loading map region: " + definition.getRegionId() + ", ids: "
              + definition.getObjectFileId() + " and " + definition.getTerrainFileId());
        }
      }
      ...
      logger.info(String.format("Unpacked: %s maps, %d map definitions",
          StringUtils.format(successfull), definitions.size()));
   ...
   * Decodes the regions
   * 
   * @param regionId The region to decode
   * 
   * @param objectData The data that contains object information
   * 
   * @param terrainData The data that contains terrain information
   * 
   * @throws Exception
   */
  private static void decodeRegion(int regionId, ByteBufReader objectData,
      ByteBufReader terrainData) throws Exception {
    ...
    while (idOffset != 0) {
      ...
      while (positionOffset != 0) {
        ...
        if (height >= 0 && height <= 3) {
          ...
          GameObject object = new GameObject(id, type,
              new Position(absX + localX, absY + localY, height), orientation);
