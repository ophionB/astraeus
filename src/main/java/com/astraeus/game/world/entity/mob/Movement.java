package com.astraeus.game.world.entity.mob;
...
import com.astraeus.net.packet.out.UpdateMapRegion;
import com.astraeus.util.Stopwatch;
...
public final class Movement {
   ...
   */
  public static final AttributeKey<Boolean> LOCK_MOVEMENT =
      AttributeKey.valueOf("lock_movement", false);
   ...
   *
   * @param isRunningQueueEnabled The new modification.
   */
  ...
  public final void reset() {
    ...
    focusPoints.clear();
    focusPoints.add(
        new MovementPoint(getEntity().getPosition().getX(), getEntity().getPosition().getY(), -1));
  }

  /**
   * Handles the cycling movement requests of the entity. This method receives prior processing to
   * the main updating procedure to ensure that all movement is registered in time to be updated in
   * synchronization with the main procedure.
   */
  public final void handleEntityMovement() {
    ...
    int deltaX = mob.getPosition().getX() - mob.getLastPosition().getRegionalX() * 8;

    int deltaY = mob.getPosition().getY() - mob.getLastPosition().getRegionalY() * 8;
  ...
  public final MovementPoint getNextPoint() {
    ...
    if (availableFocusPoint == null || availableFocusPoint.getDirection() == -1) {
      ...
      mob.setPosition(
          mob.getPosition().transform(DIRECTION_DELTA_X[availableFocusPoint.getDirection()],
              DIRECTION_DELTA_Y[availableFocusPoint.getDirection()],
  ...
  private final class MovementPoint {
     ...
     *
     * @param direction The facing {@code direction} of this entity.
     */
