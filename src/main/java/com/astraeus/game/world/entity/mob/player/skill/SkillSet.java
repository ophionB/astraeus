package com.astraeus.game.world.entity.mob.player.skill;

import java.util.concurrent.TimeUnit;

import com.astraeus.game.sound.Sound;
...
public class SkillSet {
  ...
  public void addExperience(int skillId, double experience) {
    ...
    if (skill.getMaxLevel() < 99
      ...
      final String line1 = "Congratulations! You've just advanced "
          + StringUtils.getAOrAn(skillData.toString()) + " " + skillData + " level!";
      final String line2 = "You have reached level " + skill.getMaxLevel() + "!";

      player.queuePacket(
          new ServerMessagePacket("You've reached " + StringUtils.getAOrAn(skillData.toString())
              + " " + skillData + " level of " + skill.getMaxLevel() + "."));
  ...
  public void refresh(int skillId) {
    final Skill skill = skills[skillId];
    entity.getPlayer().queuePacket(
        new UpdateSkillPacket(skill.getSkill(), skill.getLevel(), (int) skill.getExperience()));
  }
  ...
  public void setDefault() {
    for (int skill = 0; skill < skills.length; skill++) {
      this.skills[skill] =
          new Skill(skill, skill == Skill.HITPOINTS ? 10 : 1, skill == Skill.HITPOINTS ? 1154 : 0);
    }
  }
