package com.astraeus.net.codec.game;
...
import com.astraeus.net.codec.ProtocolConstants;
...
public final class GamePacketDecoder extends ByteToMessageDecoder {
  ...
  private void decodeOpcode(ByteBuf in) {
    ...
    state = size == ProtocolConstants.VARIABLE_BYTE || size == ProtocolConstants.VARIABLE_SHORT
   ...
   * 
   * @param payload The buffer containing this packets payload.
   */
  ...
  /**
   * Represents the enumerated parts of a {@link IncomingPacket} that needs to be decoded.
   */
  private enum State {
    /**
