package com.astraeus.net.codec.login;

import java.math.BigInteger;
import java.util.List;
import java.util.logging.Logger;

import com.astraeus.net.NetworkConstants;
import com.astraeus.net.codec.ByteBufUtils;
import com.astraeus.net.codec.IsaacCipher;
...
public final class LoginDecoder extends ByteToMessageDecoder {
  ...
  private void decodeHeader(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
    // 78 is the amount of readable bytes as of now, after the size of the encrypted data is read
    // then the readable bytes
    // become 77, when the value is read to indicate the size of the encrypted login block it should
    // read the same.
    ...
    if (in.readableBytes() >= 78) {
      ...
      if (encryptedLoginBlockSize != in.readableBytes()) {
        logger.info(String.format("[host= %s] encryptedLoginBlockSize != readable bytes",
  ...
  private void decodePayload(ChannelHandlerContext ctx, ByteBuf in, List<Object> out)
    ...
    if (in.isReadable(encryptedLoginBlockSize)) {
      ...
      if (magicValue != 255) {
        logger.info(String.format("[host= %s] [magic= %d] was rejected for the wrong magic value.",
            ctx.channel().remoteAddress(), magicValue));
        LoginUtils.sendResponseCode(ctx, LoginResponse.LOGIN_SERVER_REJECTED_SESSION);
        return;
      }
      ...
      if (clientVersion != 317) {
        logger.info(
            String.format("[host= %s] [version= %d] was rejected for the wrong client version.",
                ctx.channel().remoteAddress(), clientVersion));
        LoginUtils.sendResponseCode(ctx, LoginResponse.LOGIN_SERVER_REJECTED_SESSION);
        return;
      }
      ...
      if (memoryVersion != 0 && memoryVersion != 1) {
        logger.info(String.format("[host= %s] was rejected for having the memory setting.",
            ctx.channel().remoteAddress()));
        LoginUtils.sendResponseCode(ctx, LoginResponse.LOGIN_SERVER_REJECTED_SESSION);
        return;
      }
      ...
      if (expectedSize != encryptedLoginBlockSize - 41) {
        logger.info(String.format("[host= %s] was rejected for having the wrong client settings.",
            ctx.channel().remoteAddress()));
        LoginUtils.sendResponseCode(ctx, LoginResponse.LOGIN_SERVER_REJECTED_SESSION);
        return;
      }
      ...
      if (rsa != 10) {
        logger.info(String.format("[host= %s] was rejected for having the wrong rsa opcode.",
            ctx.channel().remoteAddress()));
        LoginUtils.sendResponseCode(ctx, LoginResponse.LOGIN_SERVER_REJECTED_SESSION);
        return;
      }
      ...
      if (password.length() < 6 || password.length() > 20 || username.isEmpty()
          || username.length() > 12) {
