...
public final class LoginRequestDecoder extends ByteToMessageDecoder {
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
    if (in.readableBytes() >= 2) {
      ...
      if (request != ProtocolConstants.GAME_SEVER_OPCODE
          && request != ProtocolConstants.FILE_SERVER_OPCODE) {
        logger
            .info(String.format("[host= %s] was rejected for having an invalid connection request.",
