...
public final class IncomingPacket {
   ...
   * 
   * @return {@code true} If this packet is prioritized. {@code false} otherwise.
   */
  public boolean isPrioritized() {
    return opcode == IncomingPacket.EQUIP_ITEM || opcode == IncomingPacket.ITEM_OPTION_1
        || opcode == IncomingPacket.BUTTON_CLICK;
  }

  @Override
  public String toString() {
    return String.format("[opcode= %d], [header= %s], [size= %d], [length= %d]", opcode,
        header.name(), size, getLength());
  }
