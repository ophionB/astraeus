package io.battlerune.game.event;
...
import io.battlerune.util.ClassUtils;
...
public final class UniversalEventProvider implements EventProvider {
   ...
   */
  private final Multimap<Class<? extends Event>, EventSubscriber<? super Event>> events =
  ...
  public void provideSubscriber(EventSubscriber<?> subscriber) {
    checkSubscriber(subscriber,
        annotation -> events.put(annotation.value(), (EventSubscriber<? super Event>) subscriber));
  }

  @SuppressWarnings("unchecked")
  @Override
  public void depriveSubscriber(EventSubscriber<?> subscriber) {
    checkSubscriber(subscriber, annotation -> events.remove(annotation.value(),
        (EventSubscriber<? super Event>) subscriber));
  }

  private void checkSubscriber(EventSubscriber<?> subscriber, Consumer<SubscribesTo> consumer) {
    Optional<SubscribesTo> optional =
        ClassUtils.getAnnotation(subscriber.getClass(), SubscribesTo.class);
    Preconditions.checkArgument(optional.isPresent(),
        String.format("%s is not annotated with @SubscribesTo", subscriber.getClass()));
