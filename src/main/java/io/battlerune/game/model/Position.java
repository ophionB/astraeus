...
public final class Position {
   ...
   * 
   * @param x The x coordinate on a grid.
   * @param y The y coordinate on a grid.
   */
  ...
  /**
   * Performs a check to see if the specified position is equal to the instanced one.
   * 
  ...
  /**
   * Determines if another coordinate point is within a set distance of our own.
   * 
  ...
  public Position transform(int x, int y, int h) {
    return new Position(getX() + x, getY() + y, getHeight() + h);
  }
  ...
  public String toString() {
    return height > 0 ? String.format("[x= %d y= %d h= %d]", x, y, height)
