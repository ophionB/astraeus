package io.battlerune.game.model.entity.item;
...
public class ItemContainer implements Iterable<Item> {
   ...
   *
   * @param capacity the capacity of this item container.
   * @param policy the policy of this item container.
   */
  ...
  public Spliterator<Item> spliterator() {
    return Spliterators.spliterator(items, 0, items.length,
   ...
   * @param slot the slot to add this item in ({@code -1} for any slot).
  ...
  public boolean add(Item item, int slot) {
    ...
    if ((item.definition().isStackable() || policy.equals(ItemContainerPolicy.STACK_ALWAYS))
        && !policy.equals(ItemContainerPolicy.STACK_NEVER)) {
    ...
    if (item.definition().isStackable() || policy == ItemContainerPolicy.STACK_ALWAYS
        && policy != ItemContainerPolicy.STACK_NEVER) {
   ...
   *
   * @param c the items to add to this container.
   * @return {@code true} if this container changed as a result of the operation, {@code false}
   *         otherwise.
   */
  ...
  /**
   * Attempts to add all items from {@code container} to any slots in this container.
   *
   * @param container the container of items to add to this container.
   * @return {@code true} if this container changed as a result of the operation, {@code false}
   *         otherwise.
   */
   ...
   *
   * @param item the item to remove from this container.
   * @param slot the slot to remove this item from ({@code -1} for any slot).
   ...
   *
   * @param c the items to remove from this container.
   * @return {@code true} if this container changed as a result of the operation, {@code false}
   *         otherwise.
   */
  ...
  /**
   * Attempts to remove all items from {@code items} from any slots in this container.
   *
   * @param items the items to remove from this container.
   * @return {@code true} if this container changed as a result of the operation, {@code false}
   *         otherwise.
   */
  ...
  /**
   * Attempts to replace the first occurrence of {@code oldId} with {@code newId}. If {@code oldId}
   * isn't in this container {@code newId} will not be added.
   * 
   * @param oldId the identifier to replace.
   * @param newId the identifier to replace with.
  ...
  /**
   * Attempts to replace all occurrences of {@code oldId} with {@code newId}. If {@code oldId} isn't
   * in this container {@code newId} will not be added.
   * 
   * @param oldId the identifier to replace.
   * @param newId the identifier to replace with.
   * @return {@code true} if at least one replace operation was successful, {@code false} otherwise.
   */
   ...
   * @return {@code true} if this container contains the identifier, {@code false} otherwise.
   */
   ...
   * @return {@code true} if this container contains all of the identifiers, {@code false}
   *         otherwise.
   */
   ...
   *
   * @param identifiers the identifiers to check this container for.
   * @return {@code true} if this container contains any of the identifiers, {@code false}
   *         otherwise.
   */
   ...
   *
   * @param items the items to check this container for.
   * @return {@code true} if this container contains all of the items, {@code false} otherwise.
   */
   ...
   *
   * @param items the items to check this container for.
   * @return {@code true} if this container contains any of the items, {@code false} otherwise.
   */
  ...
  /**
   * Transfers the item in {@code slot} to {@code newSlot}. If an item already exists in the new
   * slot, the items in this container will be shifted to accommodate for the transfer.
   *
   ...
   *
   * @param player the player to refresh this container for.
   * @param widget the interface to refresh the contents of this container on.
   */
  ...
  /**
   * Sets the container of items to {@code items}. The container will not hold any references to the
   * array, nor the item instances in the array.
   *
   * @param items the new array of items, the capacities of this must be equal to or lesser than the
   *        container.
   */
   ...
   *
   * @param slot the to determine if free or not.
   ...
   *
   * @param slot the to determine if used or not.
  ...
  /**
   * Places {@code item} on {@code slot} regardless of if there is an existing item on the slot or
   * not.
   *
   * @param slot the slot to place the item on.
   * @param item the item to place on the slot.
   */
   ...
   * @return the item wrapped within an optional, or an empty optional if no item was found.
   */
   ...
   *
   * @param slot the slot to get the item identifier on.
   * @return the item identifier on the slot, or {@code -1} if no item exists on the slot.
   */
   ...
   * @return the total quantity of items in this container with the identifier.
   */
  public int amount(int id) {
    return stream().filter(i -> Item.valid(i) && i.getId() == id).mapToInt(i -> i.getAmount())
        .sum();
  }
  ...
  /**
   * Gets the list of listeners in this container. Changes made to the returned list will have an
   * effect on this container.
   *
  ...
  /**
   * Retrieves the items contained within this container. Modifications made to the returned array
   * will be reflected on the backing array. Use {@link ItemContainer#containerCopy()} for a "safe"
  ...
  /**
   * Retrieves a "safe" version of the items contained within this container. Modifications made to
   * the returned array will not be reflected on the backing array. Use
   * {@link ItemContainer#container()} for a copy of the backing array that holds references.
   *
  ...
  private static final class ItemContainerIterator implements Iterator<Item> {
     ...
     * Creates a new {@link ItemContainer}.
     *
     * @param container the container that is storing the elements.
     */
