...
public class Decoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> arg2) throws Exception {
		...
		if(in.isReadable()) {
			...
			//Handle the request..
			switch(opcode) {
			case FileServer.JAGGRAB_REQUEST_OPCODE:
				decodeJagGrabRequest(ctx, in);
				break;
			case FileServer.ONDEMAND_REQUEST_OPCODE:
	...
	private void decodeJagGrabRequest(ChannelHandlerContext ctx, ByteBuf in) {
		...
		if(in.isReadable()) {
			...
			final String path = new String(pathBuffer, FileServer.JAGGRAB_CHARSET);

			//Attempt to get the file that's been requested..
			final ByteBuf file = Server.getCache().request(path);
	...
	private void decodeOnDemandRequest(ChannelHandlerContext ctx, ByteBuf in) {
		...
		if (in.readableBytes() >= Byte.BYTES + Integer.BYTES) {
			...
			ByteBuf file = null;
			try {
				file = Server.getCache().getFile(fileType, fileId);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			if(file != null) {
				...
				for (int chunk = 0; file.readableBytes() > 0; chunk++) {
					...
					if (chunkSize > FileServer.MAX_ONDEMAND_CHUNK_LENGTH_BYTES) {
						chunkSize = FileServer.MAX_ONDEMAND_CHUNK_LENGTH_BYTES;
					}
