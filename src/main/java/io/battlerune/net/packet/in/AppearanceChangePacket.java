...
import io.battlerune.game.model.entity.mob.player.Appearance;
...
import io.battlerune.net.packet.out.RemoveWidgetPacket;
...
public class AppearanceChangePacket implements Receivable {
   ...
   * 
   * @param appearance The appearance values to check.
   * 
   * @return {@code true} If the player has valid appearance values. {@code false} otherwise.
   */
    ...
    switch (appearance.getGender()) {
      ...
      case MALE:
        ...
        if (!(appearance.getHead() >= 0 && appearance.getHead() <= 8)) {
          player.queuePacket(
              new ServerMessagePacket(player.getRights().greaterOrEqual(PlayerRights.ADMINISTRATOR)
                  ? String.format("Cannot change appearance - Invalid head id %d",
                      appearance.getHead())
        ...
        if (!(appearance.getJaw() >= 10 && appearance.getJaw() <= 17)) {
          player.queuePacket(
              new ServerMessagePacket(player.getRights().greaterOrEqual(PlayerRights.ADMINISTRATOR)
                  ? String.format("Cannot change appearance - Invalid jaw id %d",
                      appearance.getJaw())
        ...
        if (!(appearance.getTorso() >= 18 && appearance.getTorso() <= 25)) {
          player.queuePacket(
              new ServerMessagePacket(player.getRights().greaterOrEqual(PlayerRights.ADMINISTRATOR)
                  ? String.format("Cannot change appearance - Invalid torso id %d",
                      appearance.getTorso())
        ...
        if (!(appearance.getArms() >= 26 && appearance.getArms() <= 31)) {
          player.queuePacket(
              new ServerMessagePacket(player.getRights().greaterOrEqual(PlayerRights.ADMINISTRATOR)
                  ? String.format("Cannot change appearance - Invalid arm id %d",
                      appearance.getHead())
        ...
        if (!(appearance.getHands() >= 33 && appearance.getHands() <= 34)) {
          player.queuePacket(
              new ServerMessagePacket(player.getRights().greaterOrEqual(PlayerRights.ADMINISTRATOR)
                  ? String.format("Cannot change appearance - Invalid hand id %d",
                      appearance.getHands())
        ...
        if (!(appearance.getLegs() >= 36 && appearance.getLegs() <= 40)) {
          player.queuePacket(
              new ServerMessagePacket(player.getRights().greaterOrEqual(PlayerRights.ADMINISTRATOR)
                  ? String.format("Cannot change appearance - Invalid leg id %d",
                      appearance.getLegs())
        ...
        if (!(appearance.getFeet() >= 42 && appearance.getFeet() <= 43)) {
          player.queuePacket(
              new ServerMessagePacket(player.getRights().greaterOrEqual(PlayerRights.ADMINISTRATOR)
                  ? String.format("Cannot change appearance - Invalid feet id %d",
                      appearance.getFeet())
  ...
  public void handlePacket(Player player, IncomingPacket packet) {
    ...
    final Appearance appearance =
        new Appearance(gender == 0 ? Appearance.Gender.MALE : Appearance.Gender.FEMALE, head,
            gender == Appearance.Gender.MALE.getCode() ? jaw : -1, torso, arms, hands, legs, feet,
