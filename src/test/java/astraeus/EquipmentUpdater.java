...
public class EquipmentUpdater {
  ...
  public static void main(String[] args) {
    ...
    List<EquipmentDefinition> equipDefs = new ArrayList<>();

    for (ItemDefinition def : ItemDefinition.definitions) {
      ...
      EquipmentType type = getEquipmentType(def);
      ...
      if (type == null) {
        System.out.println("equip type is null!");
      }

      Set<SkillRequirement> requirements = new LinkedHashSet<>();

      int attackReq = getAttackRequirement(def);

      if (attackReq != 1) {
        requirements.add(new SkillRequirement(attackReq, SkillData.ATTACK));
      }

      int strReq = getStrengthRequirement(def);

      if (strReq != 1) {
        requirements.add(new SkillRequirement(strReq, SkillData.STRENGTH));
      }

      int defenceReq = getDefenceRequirement(def);

      if (defenceReq != 1) {
        requirements.add(new SkillRequirement(defenceReq, SkillData.DEFENCE));
      }

      int magic = getMagicRequirement(def);

      if (magic != 1) {
        requirements.add(new SkillRequirement(magic, SkillData.MAGIC));
      }

      int prayer = getPrayerRequirement(def);

      if (prayer != 1) {
        requirements.add(new SkillRequirement(prayer, SkillData.PRAYER));
      }

      int range = getRangedRequirement(def);

      if (range != 1) {
        requirements.add(new SkillRequirement(range, SkillData.RANGED));
      }

      boolean fullBody = isFullBody(def);

      boolean fullHat = isFullHat(def);

      boolean fullMask = isFullMask(def);

      int[] bonuses = new int[14];

      Arrays.fill(bonuses, 0);

      equipDefs.add(new EquipmentDefinition(id, name, type,
          requirements.stream().toArray(SkillRequirement[]::new), fullBody, fullHat, fullMask,
          bonuses));
    ...
    String s = gson.toJson(equipDefs.toArray());

    try (FileWriter writer = new FileWriter(new File("equipment_definitions.json"))) {
   ...
   */
  private static EquipmentType getEquipmentType(ItemDefinition definition) {
    ...
    for (String element : CAPES) {
      if (name.contains(element)) {
        return EquipmentType.CAPE;
      }
    }
    for (String element : HATS) {
      if (name.contains(element)) {
        return EquipmentType.HAT;
      }
    }
    for (String element : BOOTS) {
      if (name.contains(element)) {
        return EquipmentType.BOOTS;
      }
    }
    for (String element : GLOVES) {
      if (name.contains(element)) {
        return EquipmentType.GLOVES;
      }
    }
    for (String element : SHIELDS) {
      if (name.contains(element)) {
        return EquipmentType.SHIELD;
      }
    }
    for (String element : AMULETS) {
      if (name.contains(element)) {
        return EquipmentType.AMULET;
      }
    }
    for (String element : ARROWS) {
      if (name.endsWith(element) || name.startsWith(element)) {
        return EquipmentType.ARROWS;
      }
    }

    outer: for (String element : RINGS) {
      for (String ex : RING_EXCEPTIONS) {
        if (name.contains(ex)) {
          break outer;
        }
      }
    ...
    for (String element : LEGS) {
      if (name.contains(element)) {
        return EquipmentType.LEGS;
      }
    }

    outer: for (String element : WEAPONS) {

      for (String ex : WEAPON_EXCEPTIONS) {
        if (name.contains(ex)) {
          break outer;
        }
      }
  ...
  private static boolean isFullBody(ItemDefinition definition) {
    ...
    for (String element : FULL_BODIES) {
      if (name.contains(element)) {
        return true;
      }
    }
   ...
   * Arrows.
   */
  public static final String[] ARROWS = {"arrow", "arrows", "arrow(p)", "arrow(+)", "arrow(s)",
      "bolt", "Bolt rack", "Opal bolts", "Dragon bolts"};

  /**
   * Bodies.
   */
  public static final String[] BODY = {"platebody", "chainbody", "robetop", "leathertop",
      "platemail", "brassard", "robe top", "body", "chestplate", "torso", "shirt"};
   ...
   * Full bodies.
   */
  public static final String[] FULL_BODIES =
      {"top", "shirt", "platebody", "Ahrims robetop", "Karils leathertop", "brassard", "Robe top",
          "robetop", "platebody (t)", "platebody (g)", "chestplate", "torso"};

  /**
   * Full hats.
   */
  public static final String[] FULL_HATS =
      {"med helm", "coif", "Dharoks helm", "hood", "Initiate helm", "Coif", "Helm of neitiznot"};

  /**
   * Full masks.
   */
  public static final String[] FULL_MASKS = {"full helm", "mask", "karils coif"};

  /**
   * Gloves.
   */
  public static final String[] GLOVES = {"gloves", "gauntlets", "vambraces", "bracers"};

  /**
   * Hats.
   */
  public static final String[] HATS =
      {"tiara", "hood", "coif", "partyhat", "hat", "cavalier", "boater", "helmet"};

  /**
   * Legs.
   */
  public static final String[] LEGS = {"platelegs", "plateskirt", "skirt", "bottoms", "chaps",
      "bottom", "skirt", "chaps", "tassets", "trousers"};
  ...
  public static final String[] RING_EXCEPTIONS = {"string"};

  /**
   * Shields.
   */
  public static final String[] SHIELDS =
      {"kiteshield", "sq shield", "toktz-ket", "books", "book", "kiteshield", "defender", "shield"};

  /**
   * Weapons.
   */
  public static final String[] WEAPONS = {"scimitar", "longsword", "sword", "longbow", "shortbow",
      "crossbow", "dagger", "mace", "halberd", "spear", "abyssal whip", "axe", "flail",
      "torags hammers", "maul", "dart", "javelin", "knife", "toktz-xil", "toktz-mej", "tzhaar-ket",
      "staff", "godsword", "c'bow", "crystal bow", "dark bow", "magic butterfly net"};


  public static final String[] WEAPON_EXCEPTIONS = {"crossbow string", "bow (u)"};
